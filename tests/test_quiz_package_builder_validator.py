...
def test_generated_multiselect_matches_native_sandbox_scoring(tmp_path):
    result = run_script('scripts/build_quiz_package_from_workbook.py',
        str(AUTHORING_FIXTURE / 'buildable_quiz.model.json'), '--settings',
        str(AUTHORING_FIXTURE / 'buildable_quiz.settings.json'), '--output-dir', str(tmp_path / 'package'))
    assert result.returncode == 0, result.stdout + result.stderr
    root = ET.parse(tmp_path / 'package/questiondb.xml').getroot()
    generated = next(i for i in root.iter('item') if any(
        e.text == 'Multi-Select' for e in i.iter('fieldentry')))
    native = ET.parse(REPO_ROOT / 'tests/fixtures/quiz_source_choice_fidelity/native_multiselect.xml').getroot()
    def shape(item):
        lid = item.find('.//response_lid').get('ident')
        ids = {e.get('ident'): f'option-{i}' for i,e in enumerate(item.iter('response_label'))}
        def visit(e):
            return (local_name(e.tag), {k: ('LID' if v == lid else v) for k,v in e.attrib.items()},
                    ids.get((e.text or '').strip(), (e.text or '').strip()), [visit(c) for c in e])
        return visit(item.find('resprocessing'))
    assert shape(generated) == shape(native)


def test_validator_rejects_generic_multiselect_profile_that_lost_native_key(tmp_path):
    sys.path.insert(0, str(REPO_ROOT / 'scripts'))
    from validate_quiz_package import ValidationReport, validate_multiselect_keys
    root = ET.parse(REPO_ROOT / 'tests/fixtures/quiz_source_choice_fidelity/quiz_d2l_choices.xml').getroot()
    report = ValidationReport(tmp_path)
    validate_multiselect_keys(root, report)
    assert any('native answer key' in error for error in report.errors)
