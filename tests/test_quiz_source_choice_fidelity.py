...
def test_native_sandbox_multiselect_key_is_recognized_without_changing_evidence():
    item = ET.parse(FIXTURE / "native_multiselect.xml").getroot()
    ...
    assert projection['scoring_mode'] == 'all_or_nothing'
    assert [o['correct'] for o in projection['options']] == [True, True, False]
    assert facts == original


@pytest.mark.parametrize('mutation', ['wrong-complement', 'missing-complement-term',
    'wrong-action', 'wrong-continuation', 'wrong-default', 'unkeyed'])
def test_native_multiselect_does_not_guess_ambiguous_keys(mutation):
    item = ET.parse(FIXTURE / "native_multiselect.xml").getroot()
    processing = item.find('resprocessing')
    positive, negative = processing.findall('respcondition')
    complement = negative.find('conditionvar/and/not')
    if mutation == 'wrong-complement': complement[0].text = complement[1].text
    elif mutation == 'missing-complement-term': complement.remove(complement[0])
    elif mutation == 'wrong-action': negative.find('setvar').set('action', 'Subtract')
    elif mutation == 'wrong-continuation': positive.set('continue', 'no')
    elif mutation == 'wrong-default': processing.find('outcomes/decvar').set('defaultval', '1')
    elif mutation == 'unkeyed':
        predicate = positive.find('conditionvar')
        terms = list(predicate.iter('varequal'))
        predicate.clear()
        ET.SubElement(predicate, 'not').extend(deepcopy(terms))
        complement.clear(); complement.extend(deepcopy(list(predicate)))
