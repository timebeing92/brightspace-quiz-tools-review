...
def declared_draw_points(draw: dict[str, Any]) -> Any:
    """Prefer an authoring override, then the native draw's observed weight.

    Library question points can differ from the points awarded by a quiz draw.
    Keep those two scopes separate, including when adding a new pool member.
    """
    value = draw["selection"]["extensions"].get("points_per_question")
    if value is None and draw.get("source_kind") == "section":
        value = draw.get("extensions", {}).get("section_weight")
    return value
